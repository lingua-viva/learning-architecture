...
- Whether v0.2.72 — the build she used on 29 August — is what a school would install today; the Italian fix is on this branch and on `origin/fix/safeguarding-italian-2026-09-02`, **not on `main`**, so no shipped build contains it yet (AGENTS.md's definition of "pushed" applies: a teacher clicking download today gets the 29 August behaviour).
