...
async function handleRequest(req, res) {
  ...
  if (path === '/api/chat' && req.method === 'POST') {
    try {
      ...
      // ── MISSION CANVAS PIPELINE (via API server on :7891) ────────────
      // The Python pipeline handles: SCAN → CLASSIFY → RETRIEVE → RESEARCH → CONTEXT → REASON → SYNTHESIZE → STORE
      // We call it and stream the results as SSE events to the browser.
      const MC_API = process.env.MC_API_URL || 'http://127.0.0.1:7891';
      let mcResult = null;
      let pipelineContext = '';
      const learnMode = mode === 'learn';

      try {
        res.write(`event: pipeline\ndata: ${JSON.stringify({ step: 'classify', status: 'start', route: 'LOCAL' })}\n\n`);
        const pipelineStart = Date.now();

        const mcResp = await fetch(`${MC_API}/api/query`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ query: text, intent: mode || null }),
          signal: AbortSignal.timeout(120000),
        });

        if (mcResp.ok) {
          mcResult = await mcResp.json();
          const cls = mcResult.classification || {};
          const pipe = mcResult.pipeline || {};
          const result = mcResult.result || {};
          ...
          res.write(`event: palette\ndata: ${JSON.stringify({
            ...
            lib_ids: pipe.knowledge_used || [],
          })}\n\n`);
          res.write(`event: pipeline\ndata: ${JSON.stringify({ step: 'classify', status: 'done', model: 'ontology', route: 'LOCAL', ms: Date.now() - pipelineStart, node: cls.node, confidence: cls.confidence })}\n\n`);

          // Send step events for each pipeline step
          for (const step of (pipe.steps || [])) {
            const stepLower = step.toLowerCase();
            if (stepLower === 'scan' || stepLower === 'classify') continue; // Already sent
            const route = (stepLower === 'research' && pipe.external_called) ? 'EXTERNAL' : 'LOCAL';
            res.write(`event: pipeline\ndata: ${JSON.stringify({ step: stepLower, status: 'done', route, ms: pipe.duration_ms })}\n\n`);
          }

          // Build context for synthesis model
          pipelineContext = result.content || '';
          if (pipe.gap_signals?.length) {
            res.write(`event: pipeline\ndata: ${JSON.stringify({ step: 'gaps', status: 'info', gaps: pipe.gap_signals })}\n\n`);
          }
          if (cls.blocks_external) {
            res.write(`event: pipeline\ndata: ${JSON.stringify({ step: 'research', status: 'blocked', route: 'BLOCKED', reason: 'Sensitive classification — no external routing' })}\n\n`);
          }
        }
      } catch (e) {
        // API server not running — fall back to direct model call without pipeline
        res.write(`event: pipeline\ndata: ${JSON.stringify({ step: 'classify', status: 'skipped', reason: 'Pipeline API unavailable' })}\n\n`);
      }

      const isInternalOnly = mcResult?.classification?.blocks_external;

      // ── PIPELINE STEP 5: SYNTHESIZE (intent-specific model) ────────
      // GOVERNANCE: if blocks_external, force local model only
      let synthConfig = config;
      if (isInternalOnly && config.provider !== 'ollama') {
        synthConfig = { provider: 'ollama', model: 'qwen2.5:3b' };
        res.write(`event: pipeline\ndata: ${JSON.stringify({ step: 'synthesize', status: 'governance_override', reason: 'Sensitive classification — forced local model', original_model: `${config.provider}/${config.model}` })}\n\n`);
      }
      const isLocal = synthConfig.provider === 'ollama';
      res.write(`event: pipeline\ndata: ${JSON.stringify({ step: 'synthesize', status: 'start', model: `${synthConfig.provider}/${synthConfig.model}`, route: isLocal ? 'LOCAL' : 'EXTERNAL' })}\n\n`);
      ...
      // Call the LLM (uses synthConfig — governance may have forced local)
      const llmStream = await callLLM(synthConfig, systemPrompt, text);
      ...
      if (mcResult?.pipeline?.external_called) modelsUsed.push('perplexity');
      modelsUsed.push(`${synthConfig.provider}/${synthConfig.model}`);
      res.write(`event: pipeline\ndata: ${JSON.stringify({
